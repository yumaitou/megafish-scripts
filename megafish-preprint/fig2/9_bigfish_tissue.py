...
ims_path = '/spo82/ana/012_SeqFISH_IF/240822/image/rna3/'
output_path = '/spo82/ana/012_SeqFISH_IF/240822/bigfish/rna3/'

files_header = [str(n) for n in range(74)]
files_footer = "_7_7.tif"

files = []
for header in files_header:
    files.append(header + files_footer)
...
def process_im(im, sigma, voxel_size_yx, psf_yx, thr, im_path, only_detect_spot, output_path):
    ...
    # Filter image LoG:
    time_tmp = time.time()
    rna_log = stack.log_filter(im.squeeze(), sigma)
