...
ims_path = '/spo82/ana/240521_simu/240807/image'
output_path = '/spo82/ana/240521_simu/240807/bigfish/'

files_header = ["simu_img_num" + str(n)
                for n in [96, 288, 960, 2881, 9604, 28812]]
files_footer = ["_rep" + str(i + 1) + "_mip.tif" for i in range(3)]

files = []
for header in files_header:
    for footer in files_footer:
        files.append(header + footer)
...
def process_im(im, sigma, voxel_size_yx, psf_yx, thr, im_path, only_detect_spot, output_path):
    ...
    # print(
    #     f'sigmas={sigma} threshold={thr:.3f}, detected #spots={spots.shape[0]}', flush=True)

    conditions_str = (f'FishQuant_results_{os.path.basename(im_path)}')

    # # save the spots (int)
    # df = pd.DataFrame(data=spots, columns=["y", "x"])
    # df = df[["x", "y"]]
    # df_path = os.path.join(
    #     output_path, f'{conditions_str}_noSubLocalization.csv')
    ...
    # print(
    #     f'Times:log{time1}_localmax{time2}_findspots{time4}_gaussfit{time5}', flush=True)
    # print(f'Times w gaussian: {time1 + time2 + time4 + time5}', flush=True)

    df = pd.DataFrame(data=spots_subpixel, columns=["y", "x"])
    df = df[["x", "y",]].round(3)
    df_path = os.path.join(
        output_path, f'{conditions_str}_withSubLocalization.csv')
    df.to_csv(df_path, index=False)
    print(f'{df_path} saved', flush=True)
...
for i, im_path in enumerate(paths_ims_to_process):
    ...
    process_im(im, sigma, voxel_size_yx,
               psf_yx, thr, im_path, only_detect_spot, output_path)

print('Total time:', time.time() - st, flush=True)

# pix Total time: 13.64416241645813
# sub Total time: 1440.3865644931793
