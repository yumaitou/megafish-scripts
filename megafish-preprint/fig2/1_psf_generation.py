...
if __name__ == '__main__':
    ...
    pitch = 0.01 * 200 / 201  # um/pix
    shape = (101, 101)  # shape * 2 - 1
    dims = (shape[0] * pitch, shape[1] * pitch)

    # Alexa488 > em 519 laser 488
    args = {
        'shape': shape,  # number of samples in z and r direction
        'dims': dims,  # size in z and r direction in micrometers
        'ex_wavelen': 488.0,  # excitation wavelength in nanometers
        'em_wavelen': 519.0,  # emission wavelength in nanometers
        'num_aperture': 1.4,
        'refr_index': 1.401,
        'magnification': 1.0,
        # 'pinhole_radius': 0.42524,  # in micrometers
        'pinhole_radius': 100,
        'pinhole_shape': 'round',
    }

    # ATTO565 > em 592 laser 561
    # args = {
    #     'shape': shape,  # number of samples in z and r direction
    #     'dims': dims,  # size in z and r direction in micrometers
    #     'ex_wavelen': 561.0,  # excitation wavelength in nanometers
    #     'em_wavelen': 592.0,  # emission wavelength in nanometers
    #     'num_aperture': 1.4,
    #     'refr_index': 1.401,
    #     'magnification': 1.0,
    #     # 'pinhole_radius': 0.48885,  # in micrometers
    #     'pinhole_radius': 100,
    #     'pinhole_shape': 'round',
    # }

    # Alexa647 > em 671 laser 637
    # args = {
    #     'shape': shape,  # number of samples in z and r direction
    #     'dims': dims,  # size in z and r direction in micrometers
    #     'ex_wavelen': 637.0,  # excitation wavelength in nanometers
    #     'em_wavelen': 671.0,  # emission wavelength in nanometers
    #     'num_aperture': 1.4,
    #     'refr_index': 1.401,
    #     'magnification': 1.0,
    #     # 'pinhole_radius': 0.5551,  # in micrometers
    #     'pinhole_radius': 100,
    #     'pinhole_shape': 'round',
    # }

    # psf_example(
    #     savebin=False, savetif=False, savevol=False, plot=False, cmap='hot', **args
    # )
    psf_example(
        savebin=True, savetif=True, savevol=True, plot=False, cmap='hot', **args
    )
